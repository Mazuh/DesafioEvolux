...
class Image(object):
    ...
    For public methods awareness, its indexes are from 1 up to
    its dimensions (inclusive ranges).
    """
    _BLANK_COLOR = 'O'
    ...
    __AVAILABLE_CLI = {
        ...
        'K': r'^K (?P<X1>[0-9]+) (?P<Y1>[0-9]+) (?P<X2>[0-9]+) (?P<Y2>[0-9]+) (?P<C>[A-Z])$',  # noqa
    ...
    def get_color(self, col: int, row: int):
        ...
        Note that this is the recommended access interface for most
        methods public or not, cause its parameters most be from 1 up to
        their limits (and NOT begining by 0)."""
    ...
    def draw_vertical(self, col: int, begining_row: int,
                      ending_row: int, color: str):
    ...
    def draw_horizontal(self, row: int, begining_col: int, ending_col: int,
                        color: str):
    ...
    def draw_rect(self, l_col: int, l_row: int, r_col: int, r_row: int,
                  color: str):
        """Draw a rectangle from a upper left coordinate up
        to a right bottom coordinate."""
        for col_i in range(l_col, r_col+1):
            for row_i in range(l_row, r_row+1):
                self.set_color(col_i, row_i, color)

    def fill(self, ref_col: int, ref_row: int, new_color: str,
             _replaceable_color=None):
        """Paint a whole homogeneous area starting by an initial
        reference coordinate."""

        if ref_col < 1 or ref_col > self.__cols_qtt \
                or ref_row < 1 or ref_row > self.__rows_qtt:
            return
    ...
    def cli_exec(self, raw_command_line: str):
        """Interpret a command line and, if it's valid, apply such command
        on this object.

        For good practices reasons, this eval function will not create
        another instance of image nor replace the instance itself,
        i.e., the self pointer will remain constant.
        That said, static method init_by_cli(str) may be useful.

        Return True if could find a valid command and its parameters,
        otherwise False.
        """
        ...
        if cmd == Image.__INIT_COMMAND_KEY:
            return False
        if cmd not in Image.__AVAILABLE_CLI:
            return False
